...
while true; do
    ...
    case $sn in
		...
		then
			...
			PGPASSWORD="postgres" sshpass -p lliege@2020#! pg_dump -h 192.168.1.45 -U postgres --port=5432 grp3 > /var/www/html/backups/grp3_lliege_producao.sql
		fi
		...
		PGPASSWORD="postgres" psql -v ON_ERROR_STOP=1 -h localhost -U postgres --port=25432 --<<-EOSQL
			...
			pg_terminate_backend(pid) 
		     ...
		     AND datname = 'grp3_lliege_producao';

		    DROP DATABASE IF EXISTS grp3_lliege_producao;
		    CREATE DATABASE grp3_lliege_producao;

		    \c grp3_lliege_producao
		    \i /var/www/html/backups/grp3_lliege_producao.sql
		EOSQL
